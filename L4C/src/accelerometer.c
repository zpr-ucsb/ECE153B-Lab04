...
void accWrite(uint8_t addr, uint8_t val) {
    ...
    // All data read will be discarded. MB bit will always be low.
		// 00 (addr)
    uint16_t transferData = (uint16_t)(~(1 << 8 | 1 << 7)) | addr;
...
uint8_t accRead(uint8_t addr) {
    ...
    uint16_t commandData = ((1 << 8) | (uint16_t)(addr));
		uint16_t transferData = (commandData << 8); //dummy second byte (all zeros)

    uint16_t receivedData = SPI_Transfer_Data(transferData);
    return receivedData;//(uint8_t)receivedData;
}
