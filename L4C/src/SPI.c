...
void SPI1_GPIO_Init(void) {
	...
	GPIOA->MODER &= ~(GPIO_MODER_MODER4);      // 10 = Alternative Function mode
  GPIOA->MODER |= GPIO_MODER_MODER4_1;        
	GPIOA->AFR[0] &= ~(GPIO_AFRL_AFSEL4);      // Choose alternative function 5
	GPIOA->AFR[0] |= GPIO_AFRL_AFSEL4_0;
  GPIOA->AFR[0] |= GPIO_AFRL_AFSEL4_2;       
	GPIOA->OTYPER &= ~(GPIO_OTYPER_OT4);      // 0 = Push-Pull
  GPIOA->PUPDR &= ~(GPIO_PUPDR_PUPD4);      // 00 = No PU PD. IS THIS WHAT NO PULL MEANS?
	GPIOA->OSPEEDR |= GPIO_OSPEEDR_OSPEED4;		// 11 = very high speed
